    ...
    <ul class="ul" ref="ul">
      <update @tijiao="getwelllist">
        <transition-group appear :duaction="500" tag="div">
          <li v-for="item in welllist" :key="item.id" @click="pushinfo(item.id)">
...
export default {
  ...
  methods: {
    getwelllist() {
      this.$http
        .get("/api/movieOnInFoList?cityId=" + this.$store.state.id)
        .then(res => {
    ...
    pushinfo(id) {
      this.$router.push({ name: "wellmovieinfo", params: { id: id } });
    }
  },
  components: {
    tabbar,
